...
import { Moon, Sun, Plane, Upload } from 'lucide-react';
    ...
    <div className="min-h-screen gradient-bg">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex h-16 items-center justify-between">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-xl btn-primary grid place-items-center shadow-lg shadow-primary/20">
                <Plane className="w-5 h-5 text-white" />
              </div>
              <div>
                <span className="text-xl font-bold tracking-tight">SkyLog Pro</span>
                <div className="text-[10px] text-accent font-medium -mt-1">CARs 401.05 VERIFIED</div>
              </div>
            </div>
            
            <div className="flex items-center gap-3">
              <button onClick={() => setShowImporter(true)} className="hidden sm:flex items-center gap-2 px-3 py-2 rounded-xl glass hover:bg-white/10 text-sm">
                <Upload className="w-4 h-4" />
                Import CSV
              </button>
              <div className="hidden sm:block text-right">
                <div className="text-sm font-medium">Viktor Pogrebniak</div>
                <div className="text-xs text-slate-400">1,247.3 hrs â¢ CYMM</div>
              </div>
              <button onClick={toggleDark} className="p-2.5 rounded-xl glass hover:bg-white/10 transition-colors">
                {dark ? <Sun className="w-5 h-5 text-accent" /> : <Moon className="w-5 h-5" />}
              </button>
            </div>
          </div>
          
          <nav className="flex gap-6 border-t border-white/5 -mx-4 px-4 sm:mx-0 sm:px-0 overflow-x-auto">
            {tabs.map(t => (
              <button
                key={t.id}
                onClick={() => setTab(t.id)}
              ...
              >
                {t.label}
                {tab === t.id && <div className="absolute bottom-0 left-0 right-0 h-0.5 bg-primary rounded-full" />}
              </button>
            ))}
          </nav>
        </div>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="animate-fade-in">
          {tab === 'dashboard' && <Dashboard flights={flights} events={events} />}
